...
skip_to_graph = int(input("¿Desea saltarse directamente al graficado del DataFrame calculado? Sí=1, No=0: "))
if skip_to_graph == 1:
    # Solicitar el path del DataFrame calculado
    df_path = input("Ingrese el path del DataFrame calculado con anterioridad: ")
...
# Obtener todas las combinaciones de 2, 3 y 4 elementos
 ...
 print("Se realizaron las combinaciones")
...
#print(particiones_dfsg)

#Filtrado de LHCO

# Filtrar eventos usando Dask
 print("inicia el filtrado")
...
#filtered_dfsg.to_csv("Filteredsignal.csv", index=False)
 print("Se filtro el sg")
 print("Ya se realizó el filtrado de ambos df")
 print("Inicia el proceso del cálculo para el df")
 func_1=input("Desea añadir el calculo de las masas invariantes si (1), no(0): ")
 ...
 if func_1=="1":
        print ("Las combinaciones disponibles a pares para la masa invariante son :")
        for i in comb_pares_names:
                 print (i)  
        option_1=input("¿Desea modificar (quitar) alguna de estas combinaciones? si (1), no(0):")
        comb_pares_names_minv = comb_pares_names.copy()
        if option_1=="1":
          while True:
            print("Copie y pegue la combinación que desee quitar a continuación (o escriba 'salir' para terminar):")
            comb_to_remove = input()
            if comb_to_remove.lower() == 'salir':
                break
            try:
                comb_to_remove = eval(comb_to_remove)
                if comb_to_remove in comb_pares_names_minv:
                    comb_pares_names_minv.remove(comb_to_remove)
                    print(f"Combinación {comb_to_remove} eliminada.")
                else:
                    print("La combinación no se encuentra en la lista.")
            except:
                print("Formato de combinación no válido. Intente nuevamente.")
        
        comb_pares_names_minv = tuple(comb_pares_names_minv)
        print("Nueva lista de combinaciones:", comb_pares_names_minv)
        ...
        print ("Las combinaciones disponibles a trios para la masa invariante son :")
        ...
        print("Nueva lista de combinaciones:", comb_trios_names_minv)
        ...
        print ("Las combinaciones disponibles a cuartetos para la masa invariante son :")
        ...
        print("Nueva lista de combinaciones:", comb_cuartetos_names_minv)
 ...
 func_2=input("Desea añadir el calculo de las masas transversas si (1), no(0): ")
 combinaciones_pares_mtrans=[]
 comb_pares_names_mtrans=[]
 if func_2=="1":
        print ("Las combinaciones disponibles a pares para la masa transversa son :")
        for i in comb_pares_names:
                print (i)  
        option_1=input("¿Desea modificar (quitar) alguna de estas combinaciones? si (1), no(0):")
        ...
        if option_1=="1":
          ...
          print("Nueva lista de combinaciones:", comb_pares_names_mtrans)
 ...
 func_3=input("Desea añadir el calculo de la DeltaR si (1), no(0): ")
 combinaciones_pares_deltar=[]
 comb_pares_names_deltar=[]
 if func_3=="1":
        print ("Las combinaciones disponibles a pares para el calculo de deltaR son :")
        for i in comb_pares_names:
                print (i)  
        option_1=input("¿Desea modificar (quitar) alguna de estas combinaciones, si (1), no(0):?")
        ...
        if option_1=="1":
          ...
          print("Nueva lista de combinaciones:", comb_pares_names_deltar)
 ...
 func_4=input("Desea añadir el calculo del No_jets por evento si (1), no(0): ")
...
#MASA INVARIANTE
 def m_inv(evento, comb):
    ...
    if all(not p.empty for p in prt):
        ...
        return m_in
    return 0
 def calculos_eventos(df, lista_num, combinaciones_pares_minv, combinaciones_pares_deltar, combinaciones_pares_mtrans, combinaciones_trios_minv, combinaciones_cuartetos_minv, batch_size=300):
    masainv = []
    masainv_trios = []
    masainv_cuartetos = []
    masatrans = []
...
def main():
    ...
    XGBval = []
    LumiVal = []
    for IntLumi in range(300, 3100, 100):  # Loop for IntLumi from 300 to 3000 in steps of 100
        #print(f'\nLuminosity = {IntLumi}')
        xgbi = 0.5
        for jj in range(100):
    ...
    print(f"Resultados guardados en ", sig_nom )
    ...
    titulo=input("Ingrese a continuacion el titulo que desea ingresar a la grafica (contendrá luminosidad, significancia y corte de xgb como variables): ")
